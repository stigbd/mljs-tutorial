...
const names = ['sepalLength', 'sepalWidth', 'petalLength', 'petalWidht', 'type']; // For headings
...
.on('done', (error) => {
  separationSize = 0.67 * data.length;
...
function test() {
  ...
  console.log(`Test Set Size = ${testSetLength} and number of Misclassicfications = ${predictionError}`);
  predict();
}
...
function predict() {
  ...
  prompt.get(['Sepal Length', 'Sepal Width', 'Petal Length', 'Petal Widht'], function (err, result) {
    if (!err) {
